...
import { PRODUCT_STATUS_LABELS, PRODUCT_STATUS_ORDER } from "@/lib/constants";
import { PRODUCT_TRANSITIONS } from "@/lib/status-machine/product-statuses";
import { ProductStatus } from "@prisma/client";

export function ProductStatusChanger({
  productId,
  currentStatus,
}: {
  productId: string;
  ...
  const allowedNext = PRODUCT_TRANSITIONS[currentStatus];

  async function move(toStatus: ProductStatus) {
    setError(null);
      ...
      {open && (
        <div className="absolute right-0 z-10 mt-2 w-72 rounded-xl border border-slate-200 bg-white p-3 shadow-lg">
          ...
          <div className="mb-3 space-y-1">
            {PRODUCT_STATUS_ORDER.filter((s) => s !== currentStatus).map((s) => {
