...
describe("generatePaymentsForOrder — потеря копеек при округлении (фиксация текущего поведения)", () => {
  it("33/33/34 от 100.01 даёт сумму 100.00, а НЕ 100.01 (остаток теряется)", () => {
    // TODO: выглядит как баг — последний платёж не добирает остаток до batchCost.
    const order = makeOrder({ paymentTerms: "33/33/34", batchCost: new Prisma.Decimal("100.01") });
    const result = generatePaymentsForOrder(order);

    expect(result.map((p) => p.amount.toString())).toEqual(["33", "33", "34"]);

    const total = result.reduce((acc, p) => acc.add(p.amount), new Prisma.Decimal(0));
    expect(total.toString()).toBe("100");
    expect(total.eq(new Prisma.Decimal("100.01"))).toBe(false);
  });
});
