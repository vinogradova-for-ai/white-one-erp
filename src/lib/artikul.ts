...
// Артикул (vendorCode) печатается на наклейке Честного знака на производстве — китаец
// должен прочитать, поэтому код латиницей. Поле «Цвет» в карточке ЧЗ выбирается из его
// закрытого справочника (77 значений), но в АРТИКУЛ идёт только РАБОЧАЯ ПАЛИТРА White One —
// 23 цвета, которые бренд реально шьёт (финализировано Алёной 2026-06-08). Так цвет в
// карточке и код в артикуле совпадают 1:1.
// Цвет вне этих 23 (песочный, графит, пудровый, индиго…) уходит в транслит (pesochniy, …)
// — не ломается, просто не из палитры. Ключи нормализованы (нижний регистр, ё→е).
// ⚠️ голубой=blue, синий=darkblue — историческая схема White One (НЕ англ-буквальная).
