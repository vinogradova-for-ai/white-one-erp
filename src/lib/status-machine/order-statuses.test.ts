...
const ORDER_SEQUENCE: OrderStatus[] = [
  ...
  "SEWING",
  "QC",
  // READY_SHIP выпилен 04.07 («только ОТК») — в живой цепочке шага нет
  "IN_TRANSIT",
  "WAREHOUSE_MSK",
  "PACKING",
  "SHIPPED_WB",
...
describe("ORDER_TRANSITIONS — таблица переходов вперёд", () => {
  ...
  it("покрывает все статусы из enum OrderStatus", () => {
    // + READY_SHIP: легаси-точка (выпилен 04.07, в enum остался для истории логов)
    expect(Object.keys(ORDER_TRANSITIONS).sort()).toEqual(
      [...ORDER_SEQUENCE, "READY_SHIP"].sort(),
    );
  });
  ...
  it("QC идёт вперёд сразу в Доставку (READY_SHIP выпилен 04.07) и обратно на пошив", () => {
    expect(ORDER_TRANSITIONS.QC).toEqual(["IN_TRANSIT", "SEWING"]);
  });
...
describe("canMoveOrderStatus — допустимые переходы вперёд", () => {
  ...
  it("QC → IN_TRANSIT (вперёд, READY_SHIP выпилен) разрешён, без комментария", () => {
    expect(canMoveOrderStatus("QC", "IN_TRANSIT", "ASSISTANT")).toEqual({
      ok: true,
    });
  });
});
...
describe("canMoveOrderStatus — проверка всех пар from≠to матрицей", () => {
  it("ровно ожидаемые пары дают requiresComment=true (rollback руководителем)", () => {
    ...
    // Откатов назад: для индекса i (0..8) число статусов строго левее = i.
    // Но QC→SEWING НЕ откат (он в таблице) → вычитаем 1.
    // Сумма i по всем статусам = 0+1+...+8 = 36, минус 1 = 35.
    expect(rollbackForOwner).toBe(35);
    // Табличные переходы (ok без комментария): 8 линейных + QC→SEWING
    // + PREPARATION→SEWING (прямой путь в обход FABRIC_ORDERED) = 10.
