...
// Кнопка «⤓ XLSX» — выгрузка справочника ЧЗ в Excel (тот же набор колонок,
// что и в таблице). Паттерн повторяет /warehouse и /incoming.
export function ExportButton() {
  ...
  async function handleExport() {
    setLoading(true);
    try {
      const res = await fetch("/api/honest-sign/export");
      if (!res.ok) {
      ...
      const fileName = m?.[1] ?? `honest-sign-${new Date().toISOString().slice(0, 10)}.xlsx`;
