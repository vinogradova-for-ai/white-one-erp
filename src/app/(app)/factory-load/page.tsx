...
export default async function FactoryLoadPage() {
  ...
  for (const o of orders) {
    if (!o.factoryId) continue;
    if (!matrix[o.factoryId]) continue;
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-500">Суммарное количество в заказах по месяцу запуска продаж</p>
      </div>
        ...
        <table className="min-w-full divide-y divide-slate-200 text-sm">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="px-3 py-2 text-right text-xs font-semibold uppercase text-slate-500">Мощность/мес</th>
              {months.map((m) => (
            ...
            {factories.map((f) => (
              <tr key={f.id} className="hover:bg-slate-50">
                ...
                {months.map((m) => {
                  ...
                  const overload = f.capacityPerMonth && qty > f.capacityPerMonth;
                  const pct = f.capacityPerMonth ? (qty / f.capacityPerMonth) * 100 : null;
                  return (
                    <td
                      key={m}
                      className={`px-3 py-2 text-right text-xs ${
                        overload ? "bg-red-50 font-medium text-red-700" : pct && pct > 70 ? "bg-amber-50 text-amber-700" : ""
                      }`}
                    >
